...
class Mistral extends AbstractServiceProvider
{
    protected function performValidation(array $credentials): ValidationResult
    {
        ...
        try {
            ...
            $response = Prism::structured()
                ...
                ->withSchema($schema)
                ->withPrompt('Validate this API key by responding with: status="success", provider="Mistral", is_valid=true, message="API key is valid and working"')
                ->withClientOptions([
                    'timeout' => 30,
                    'connect_timeout' => 10,
                ])
            ...
            if ($validationData && $validationData['is_valid']) {
                ...
                return ValidationResult::failure(
                    provider: 'Mistral',
                    message: 'Mistral API key validation failed.',

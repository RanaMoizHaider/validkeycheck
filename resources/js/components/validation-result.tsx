...
import type { MetadataValue, ValidationResult } from '@/types/validator';
import { ValidationStatus } from '@/types/validator';
import { AlertTriangle, Ban, Check, Clock, X } from 'lucide-react';
...
export default function ValidationResult({ result, className }: ValidationResultProps) {
    ...
    return (
        <div className={cn('rounded-lg border p-4 transition-colors', colors.border, colors.bg, className)}>
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className={cn('flex h-8 w-8 items-center justify-center rounded-full', colors.iconBg, colors.iconColor)}>
                        {getStatusIcon(result.status)}
                    </div>
                    <div>
                        <h3 className={cn('text-base font-semibold', colors.titleColor)}>{result.status_label}</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{result.provider}</p>
                    </div>
                </div>

                {result.code && <div className={cn('rounded px-2 py-1 text-xs font-medium', colors.badgeBg, colors.badgeColor)}>{result.code}</div>}
            </div>

            <p className={cn('mb-3 text-sm leading-relaxed', colors.messageColor)}>{result.message}</p>

            {result.metadata && Object.keys(result.metadata).length > 0 && (
                <div className="border-t border-gray-200 pt-3 dark:border-gray-700">
                    <h4 className="mb-3 text-sm font-medium text-black dark:text-white">Additional Information</h4>
                    <div className="space-y-2">
                        {Object.entries(result.metadata).map(([key, value]) => (
                            <div key={key} className="flex items-start justify-between">
                                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{formatKeyName(key)}:</span>
                                <span className="max-w-xs text-right text-sm text-black dark:text-white">{formatValue(value)}</span>
                            </div>
                        ))}
                    </div>
                </div>

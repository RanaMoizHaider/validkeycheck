...
import ProviderCard, { ApiKeyData, ApiKeyField, ProviderCardData } from '@/components/provider-card';
    ...
    return providers.map((provider) => {
        ...
        if (Array.isArray(provider.required_fields)) {
            ...
            fields = Object.entries(provider.required_fields).map(([fieldName, displayLabel]) => ({
                ...
                placeholder: ``,
                type:
                    fieldName.toLowerCase().includes('secret') || fieldName.toLowerCase().includes('key') || fieldName.toLowerCase().includes('token')
                        ? ('password' as const)
                        : ('text' as const),
                required: true,
            }));
...
export default function Validator({ categories, currentYear, result }: ValidatorProps) {
    ...
    const filteredProviders = providerCards.filter(
        (provider) =>
            provider.name.toLowerCase().includes(searchQuery.toLowerCase()) || provider.category.toLowerCase().includes(searchQuery.toLowerCase()),
    );
    ...
    const handleValidation = async (providerId: string): Promise<void> => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setApiKeys((prev) => prev.map((key) => (key.provider === providerId ? { ...key, status: newStatus } : key)));
            } else {
            ...
            setApiKeys((prev) => prev.map((key) => (key.provider === providerId ? { ...key, status: 'invalid' } : key)));
        } finally {
            setTestingProvider(null);
        }
    };
            ...
            <div className="flex min-h-screen flex-col bg-white text-black dark:bg-black dark:text-white">
                ...
                <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-6">
                            ...
                            <div className="rounded-lg border border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-900">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Validating your API key...</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {validationResult && !testingProvider && (
                        <div>
                            <h3 className="mb-4 text-lg font-medium text-black dark:text-white">Validation Result</h3>
